...
fn test_full_time() {
    ...
    let mut search_img = rrmqr::PreparedImage::prepare(jpg);
...
fn test_full_large_version() {
    ...
    let mut search_img = rrmqr::PreparedImage::prepare(gif);
    let grids = search_img.detect_grids();
    assert_eq!(grids.len(), 1);

    let (meta, content) = grids[0].decode().unwrap();
    assert_eq!(meta.version, rrmqr::Version(14));
    assert_eq!(
        content,
...
fn test_full_multi() {
    ...
    let mut search_img = rrmqr::PreparedImage::prepare(png);
    let grids = search_img.detect_grids();
    assert_eq!(grids.len(), 3);

    let (meta, content) = grids[0].decode().unwrap();
    assert_eq!(meta.version, rrmqr::Version(13));
    assert_eq!(
        content,
    ...
    assert_eq!(meta.version, rrmqr::Version(11));
    assert_eq!(
        content,
...
fn test_mirrored() {
    ...
    let mut search_img = rrmqr::PreparedImage::prepare(img);
    let grids = search_img.detect_grids();
    assert_eq!(grids.len(), 1);

    let (meta, raw) = grids[0].decode().unwrap();
    assert_eq!(meta.version, rrmqr::Version(1));
